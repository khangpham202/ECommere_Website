  ...
  (async () => {
    if (token1) {
      try {
        ...
        let myArr = [];
        for (let i of listOrderItems) {
          for (let j of i.orderDetail) {
            myArr.push(j);
            var groupByOrderId = myArr.reduce((group, id) => {
              const { orderId } = id;
              group[orderId] = group[orderId] ?? [];
              group[orderId].push(id);
              return group;
            }, {});
          }
        }
        ...
        let orderItemPrice = "";
        for (const [key, value] of Object.entries(groupByOrderId)) {
          let a = value[0].total_money;
          ...
          if (value.length == 1) {
            ...
            for (let i of value) {
              ...
              <div class="container order mt-4">
                  ...
                  <div class="order__content--header">
                      <div class="order-id">
                          <span>Mã đơn hàng:${i.order_id}</span>
                      </div>
                      <div class="status">
                          <span><i class="fa-solid fa-hourglass-half"></i>Chờ Xác Nhận</span>
                      </div>
                  </div>
                  <hr>
                  ...
                  <hr>
                  <div class="order__content--footer">
                      <div class="btn_detail">
                        <button onclick="location.href='order/${i.orderId}'" type="button" class="btn btn-info">Chi tiết</button>
                      </div>
                      <div>
          ...
          if (value.length > 1) {
            ...
            <div class="container order mt-4">
              <div class="order__content" id="order__content">
                  <div class="order__content--header">
                      <div class="order-id">
                          <span>Mã đơn hàng:${value[0].order_id}</span>
                      </div>
                      <div class="status">
                          <span><i class="fa-solid fa-hourglass-half"></i>Chờ Xác Nhận</span>
                      </div>
            ...
            orderItemPrice += `  
              <div class="order__content--footer">
                <div class="btn_detail">
                  <button onclick="location.href='order/${value[0].order_id}'" type="button" class="btn btn-info">Chi tiết</button>
                </div>
                <div>
                  <span>Tổng số tiền: </span>
                  <span class="number">
        ...
        myOrder.append(myOrderList);
        let orderItem = $("#order__content");
        orderItem.append(orderItemContent);
        orderItem.append(orderItemPrice);
      } catch (error) {
        console.log(error);
      }
    }
